...
describe("getCacheTtlMs", () => {
  it("returns provider-specific TTL values", () => {
    expect(getCacheTtlMs("autocheck")).toBe(AUTOCHECK_CACHE_TTL_MS);
    expect(getCacheTtlMs("marketcheck")).toBe(MARKETCHECK_CACHE_TTL_MS);
    expect(AUTOCHECK_CACHE_TTL_MS).toBe(90 * 24 * 60 * 60 * 1000);
...
describe("isCacheFresh", () => {
  ...
  it("returns false when fetchedAt is missing or invalid", () => {
    expect(isCacheFresh(null, "autocheck", now)).toBe(false);
    expect(isCacheFresh(undefined, "marketcheck", now)).toBe(false);
    expect(isCacheFresh("not-a-date", "autocheck", now)).toBe(false);
  });
  ...
  it("returns true inside TTL and false when expired", () => {
    ...
    expect(isCacheFresh(fresh, "autocheck", now)).toBe(true);
    expect(isCacheFresh(stale, "autocheck", now)).toBe(false);
  ...
  it("accepts Date instances", () => {
    ...
    expect(isCacheFresh(fetchedAt, "autocheck", now)).toBe(true);
  });

  it("returns false for future timestamps", () => {
    const future = new Date("2026-06-07T12:00:00.000Z");
    expect(isCacheFresh(future, "autocheck", now)).toBe(false);
  });
});
